...
function DesignandAnalysisofAlgorithimsChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, 'SecondYear/FourthSemester/BTCOC401:Design & Analysis of Algorithms'));
                if (snapshot.exists()) {
    ...
    useEffect(() => {
        if (chartData) {
            ...
            const ctx = document.getElementById("design&analysisofalgorithmsChart").getContext("2d");
            chartRef.current = new Chart(ctx, {
                ...
                options: {
                    plugins: {
                        title: {
                            display: true,
                            text: "BTCOC401:Design & Analysis of Algorithms Ratings",
                            font: {
    ...
    const handleDownload = () => {
        ...
        XLSX.writeFile(workbook, "BTCOC401:Design & Analysis of Algorithms Feedback.xlsx");
    };
    ...
    return (
        <div style={{ width: "100%", height: "500px" }}>
            <canvas id="design&analysisofalgorithmsChart"></canvas>
            <button style={buttonStyle} onClick={handleDownload}>Download Feedback Data</button>
        </div>
    );
}

export default DesignandAnalysisofAlgorithimsChart;

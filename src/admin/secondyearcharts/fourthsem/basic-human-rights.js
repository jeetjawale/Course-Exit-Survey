...
function BasichumanrightsChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, 'SecondYear/FourthSemester/BTHM403:Basic Human Rights'));
                if (snapshot.exists()) {
    ...
    useEffect(() => {
        if (chartData) {
            ...
            const ctx = document.getElementById("basichumanrightsChart").getContext("2d");
            chartRef.current = new Chart(ctx, {
                ...
                options: {
                    plugins: {
                        title: {
                            display: true,
                            text: "BTHM403:Basic Human Rights Ratings",
                            font: {
    ...
    const handleDownload = () => {
        ...
        XLSX.writeFile(workbook, "BTHM403:Basic Human Rights Feedback.xlsx");
    };

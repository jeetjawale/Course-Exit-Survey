...
function DLDMChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, 'SecondYear/FourthSemester/BTES405:Digital Logic Design & Microprocessors'));
                if (snapshot.exists()) {
    ...
    useEffect(() => {
        if (chartData) {
            ...
            const ctx = document.getElementById("dldmChart").getContext("2d");
            chartRef.current = new Chart(ctx, {
                ...
                options: {
                    plugins: {
                        title: {
                            display: true,
                            text: "BTES405:Digital Logic Design & Microprocessors Ratings",
                            font: {
    ...
    const handleDownload = () => {
        ...
        XLSX.writeFile(workbook, "BTES405:Digital Logic Design & Microprocessors Feedback.xlsx");
    };

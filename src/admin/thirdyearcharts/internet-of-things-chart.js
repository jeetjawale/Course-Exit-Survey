...
function InternetOfThingsChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, 'ThirdYear/SixthSemester/BTCOE604(B):Internet of Things'));
                if (snapshot.exists()) {
    ...
    useEffect(() => {
        if (chartData) {
            ...
            const ctx = document.getElementById("internetofthingsChart").getContext("2d");
...
export default InternetOfThingsChart;

...
function TheoryOfComputationChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, 'ThirdYear/FifthSemester/ВТСOC502:Theory Of Computation'));
                if (snapshot.exists()) {
    ...
    useEffect(() => {
        if (chartData) {
            ...
            const ctx = document.getElementById("theoryofcomputationChart").getContext("2d");
            chartRef.current = new Chart(ctx, {
                ...
                options: {
                    plugins: {
                        title: {
                            display: true,
                            text: "BTCOC502:Theory Of Computation Ratings",
                            font: {
    ...
    return (
        <div style={{ width: "100%", height: "500px" }}>
            <canvas id="theoryofcomputationChart"></canvas>
        </div>
    );
}

export default TheoryOfComputationChart;

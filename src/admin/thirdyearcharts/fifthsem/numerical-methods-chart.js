...
function NumericalMethodsChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snapshot = await get(ref(database, 'ThirdYear/FifthSemester/ВТСОЕ504(B):Numerical Methods'));
                if (snapshot.exists()) {
    ...
    useEffect(() => {
        if (chartData) {
            ...
            const ctx = document.getElementById("numericalmethodsChart").getContext("2d");
...
export default NumericalMethodsChart;
